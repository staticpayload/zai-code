"use strict";
// ============================================================================
// ZCODE OUTPUT THEMING - Cosmic Orange
// ============================================================================
Object.defineProperty(exports, "__esModule", { value: true });
exports.tuiTags = exports.icons = void 0;
exports.action = action;
exports.thinking = thinking;
exports.reading = reading;
exports.writing = writing;
exports.deleting = deleting;
exports.success = success;
exports.error = error;
exports.warning = warning;
exports.info = info;
exports.dim = dim;
exports.hint = hint;
exports.section = section;
exports.subsection = subsection;
exports.item = item;
exports.lastItem = lastItem;
exports.keyValue = keyValue;
exports.planHeader = planHeader;
exports.planStep = planStep;
exports.fileOperation = fileOperation;
exports.fileResult = fileResult;
exports.diffHeader = diffHeader;
exports.diffLine = diffLine;
exports.progressBar = progressBar;
exports.spinner = spinner;
exports.summary = summary;
exports.response = response;
exports.responseTui = responseTui;
exports.taskType = taskType;
exports.box = box;
const settings_1 = require("./settings");
// ANSI color codes - Cosmic Orange palette
function getColors() {
    const enabled = (0, settings_1.shouldShowColor)();
    return {
        reset: enabled ? '\x1b[0m' : '',
        dim: enabled ? '\x1b[2m' : '',
        bold: enabled ? '\x1b[1m' : '',
        italic: enabled ? '\x1b[3m' : '',
        underline: enabled ? '\x1b[4m' : '',
        // Foreground - Cosmic Orange palette
        black: enabled ? '\x1b[30m' : '',
        red: enabled ? '\x1b[31m' : '',
        green: enabled ? '\x1b[32m' : '',
        yellow: enabled ? '\x1b[33m' : '',
        blue: enabled ? '\x1b[34m' : '',
        magenta: enabled ? '\x1b[35m' : '',
        cyan: enabled ? '\x1b[36m' : '',
        white: enabled ? '\x1b[37m' : '',
        gray: enabled ? '\x1b[90m' : '',
        // Bright - Orange tones (using 256 color)
        orange: enabled ? '\x1b[38;5;208m' : '', // Main orange
        brightOrange: enabled ? '\x1b[38;5;214m' : '', // Bright orange
        deepOrange: enabled ? '\x1b[38;5;202m' : '', // Deep orange
        gold: enabled ? '\x1b[38;5;220m' : '', // Gold accent
        coral: enabled ? '\x1b[38;5;209m' : '', // Coral
        peach: enabled ? '\x1b[38;5;216m' : '', // Soft peach
        rust: enabled ? '\x1b[38;5;166m' : '', // Rust
        // Cosmic accents
        purple: enabled ? '\x1b[38;5;135m' : '', // Cosmic purple
        pink: enabled ? '\x1b[38;5;212m' : '', // Cosmic pink
        // Bright foreground
        brightRed: enabled ? '\x1b[91m' : '',
        brightGreen: enabled ? '\x1b[92m' : '',
        brightYellow: enabled ? '\x1b[93m' : '',
        brightBlue: enabled ? '\x1b[94m' : '',
        brightMagenta: enabled ? '\x1b[95m' : '',
        brightCyan: enabled ? '\x1b[96m' : '',
        // Background
        bgBlack: enabled ? '\x1b[40m' : '',
        bgRed: enabled ? '\x1b[41m' : '',
        bgGreen: enabled ? '\x1b[42m' : '',
        bgYellow: enabled ? '\x1b[43m' : '',
        bgBlue: enabled ? '\x1b[44m' : '',
        bgMagenta: enabled ? '\x1b[45m' : '',
        bgCyan: enabled ? '\x1b[46m' : '',
        bgGray: enabled ? '\x1b[100m' : '',
        bgOrange: enabled ? '\x1b[48;5;208m' : '',
    };
}
const c = () => getColors();
// ============================================================================
// ACTION ICONS - ASCII symbols for different operations
// ============================================================================
exports.icons = {
    // Status
    success: '✓',
    error: '✗',
    warning: '!',
    info: '·',
    pending: '○',
    active: '●',
    // Actions
    thinking: '◐',
    reading: '>',
    writing: '+',
    modifying: '~',
    deleting: '-',
    searching: '/',
    // Flow
    arrow: '→',
    arrowDown: '↓',
    arrowUp: '↑',
    bullet: '•',
    dash: '─',
    pipe: '│',
    corner: '└',
    tee: '├',
    // Progress
    spinnerFrames: ['◐', '◓', '◑', '◒'],
    progressFull: '█',
    progressEmpty: '░',
    progressHalf: '▓',
};
// ============================================================================
// FORMATTED OUTPUT FUNCTIONS
// ============================================================================
// Action line with icon - main output format
function action(icon, label, detail) {
    const colors = c();
    const detailStr = detail ? ` ${colors.dim}${detail}${colors.reset}` : '';
    return `${colors.orange}${icon}${colors.reset} ${colors.bold}${label}${colors.reset}${detailStr}`;
}
// Thinking/processing indicator
function thinking(message = 'Thinking') {
    const colors = c();
    return `${colors.orange}${exports.icons.thinking}${colors.reset} ${colors.dim}${message}...${colors.reset}`;
}
// Reading file indicator
function reading(filePath) {
    const colors = c();
    return `${colors.gray}${exports.icons.reading}${colors.reset} ${colors.dim}Reading${colors.reset} ${colors.white}${filePath}${colors.reset}`;
}
// Writing file indicator
function writing(filePath, isNew = false) {
    const colors = c();
    const label = isNew ? 'Creating' : 'Writing';
    const icon = isNew ? exports.icons.writing : exports.icons.modifying;
    const color = isNew ? colors.green : colors.gold;
    return `${color}${icon}${colors.reset} ${colors.dim}${label}${colors.reset} ${colors.white}${filePath}${colors.reset}`;
}
// Deleting file indicator
function deleting(filePath) {
    const colors = c();
    return `${colors.red}${exports.icons.deleting}${colors.reset} ${colors.dim}Deleting${colors.reset} ${colors.white}${filePath}${colors.reset}`;
}
// Success message
function success(message) {
    const colors = c();
    return `${colors.green}${exports.icons.success}${colors.reset} ${message}`;
}
// Error message
function error(message) {
    const colors = c();
    return `${colors.red}${exports.icons.error}${colors.reset} ${message}`;
}
// Warning message
function warning(message) {
    const colors = c();
    return `${colors.gold}${exports.icons.warning}${colors.reset} ${message}`;
}
// Info message (subtle)
function info(message) {
    const colors = c();
    return `${colors.orange}${exports.icons.info}${colors.reset} ${message}`;
}
// Dim/muted text
function dim(message) {
    const colors = c();
    return `${colors.dim}${message}${colors.reset}`;
}
// Hint for next action
function hint(message) {
    const colors = c();
    return `${colors.dim}${exports.icons.arrow} ${message}${colors.reset}`;
}
// ============================================================================
// STRUCTURED OUTPUT - Sections and boxes
// ============================================================================
// Section header with line
function section(title) {
    const colors = c();
    return `\n${colors.bold}${colors.orange}${title}${colors.reset}`;
}
// Subsection with bullet
function subsection(title) {
    const colors = c();
    return `${colors.dim}${exports.icons.bullet}${colors.reset} ${colors.bold}${title}${colors.reset}`;
}
// Indented item (for lists)
function item(text, indent = 1) {
    const colors = c();
    const pad = '  '.repeat(indent);
    return `${pad}${colors.dim}${exports.icons.tee}${colors.reset} ${text}`;
}
// Last item in a list
function lastItem(text, indent = 1) {
    const colors = c();
    const pad = '  '.repeat(indent);
    return `${pad}${colors.dim}${exports.icons.corner}${colors.reset} ${text}`;
}
// Key-value pair
function keyValue(key, value) {
    const colors = c();
    return `${colors.dim}${key}:${colors.reset} ${colors.white}${value}${colors.reset}`;
}
// ============================================================================
// PLAN OUTPUT
// ============================================================================
function planHeader(stepCount) {
    const colors = c();
    return `${colors.orange}${exports.icons.bullet}${colors.reset} ${colors.bold}Plan${colors.reset} ${colors.dim}(${stepCount} steps)${colors.reset}`;
}
function planStep(index, description, status = 'pending') {
    const colors = c();
    const statusIcons = {
        pending: `${colors.dim}${exports.icons.pending}${colors.reset}`,
        active: `${colors.orange}${exports.icons.active}${colors.reset}`,
        done: `${colors.green}${exports.icons.success}${colors.reset}`,
        error: `${colors.red}${exports.icons.error}${colors.reset}`,
    };
    const icon = statusIcons[status];
    const num = `${colors.dim}${index}.${colors.reset}`;
    return `  ${icon} ${num} ${description}`;
}
// ============================================================================
// FILE OPERATIONS OUTPUT
// ============================================================================
function fileOperation(operation, filePath) {
    const colors = c();
    const ops = {
        create: { icon: exports.icons.writing, color: colors.green, label: 'create' },
        modify: { icon: exports.icons.modifying, color: colors.gold, label: 'modify' },
        delete: { icon: exports.icons.deleting, color: colors.red, label: 'delete' },
    };
    const op = ops[operation];
    return `${op.color}${op.icon}${colors.reset} ${colors.dim}${op.label}${colors.reset} ${filePath}`;
}
function fileResult(operation, filePath, success) {
    const colors = c();
    const icon = success ? `${colors.green}${exports.icons.success}${colors.reset}` : `${colors.red}${exports.icons.error}${colors.reset}`;
    return `  ${icon} ${filePath}`;
}
// ============================================================================
// DIFF OUTPUT
// ============================================================================
function diffHeader(filePath) {
    const colors = c();
    return `${colors.bold}${colors.cyan}--- ${filePath}${colors.reset}`;
}
function diffLine(line) {
    const colors = c();
    if (line.startsWith('+')) {
        return `${colors.green}${line}${colors.reset}`;
    }
    else if (line.startsWith('-')) {
        return `${colors.red}${line}${colors.reset}`;
    }
    else if (line.startsWith('@')) {
        return `${colors.cyan}${line}${colors.reset}`;
    }
    return `${colors.dim}${line}${colors.reset}`;
}
// ============================================================================
// PROGRESS INDICATORS
// ============================================================================
function progressBar(current, total, width = 20) {
    const colors = c();
    const percent = Math.min(1, Math.max(0, current / total));
    const filled = Math.round(width * percent);
    const empty = width - filled;
    const bar = exports.icons.progressFull.repeat(filled) + exports.icons.progressEmpty.repeat(empty);
    const pct = Math.round(percent * 100);
    return `${colors.orange}${bar}${colors.reset} ${colors.dim}${pct}%${colors.reset}`;
}
function spinner(frame) {
    const colors = c();
    const f = exports.icons.spinnerFrames[frame % exports.icons.spinnerFrames.length];
    return `${colors.orange}${f}${colors.reset}`;
}
// ============================================================================
// SUMMARY OUTPUT
// ============================================================================
function summary(stats) {
    const colors = c();
    const parts = [];
    if (stats.created && stats.created > 0) {
        parts.push(`${colors.green}+${stats.created} created${colors.reset}`);
    }
    if (stats.modified && stats.modified > 0) {
        parts.push(`${colors.gold}~${stats.modified} modified${colors.reset}`);
    }
    if (stats.deleted && stats.deleted > 0) {
        parts.push(`${colors.red}-${stats.deleted} deleted${colors.reset}`);
    }
    if (stats.errors && stats.errors > 0) {
        parts.push(`${colors.red}${stats.errors} errors${colors.reset}`);
    }
    if (parts.length === 0) {
        return `${colors.dim}No changes${colors.reset}`;
    }
    return parts.join(` ${colors.dim}|${colors.reset} `);
}
// ============================================================================
// RESPONSE FORMATTING
// ============================================================================
// Format AI response text with proper styling for blessed TUI
function response(text) {
    const colors = c();
    return text
        .split('\n')
        .map(line => {
        // Code blocks - dim them
        if (line.startsWith('```')) {
            return `${colors.dim}${line}${colors.reset}`;
        }
        // Headers (# ## ###) - bold orange
        if (line.match(/^#{1,3}\s/)) {
            return `${colors.bold}${colors.orange}${line.replace(/^#+\s*/, '')}${colors.reset}`;
        }
        // Bullet points - replace with our icon
        if (line.match(/^\s*[-*]\s/)) {
            return line.replace(/^(\s*)([-*])(\s)/, `$1${colors.orange}${exports.icons.bullet}${colors.reset}$3`);
        }
        // Numbered lists - dim the number
        if (line.match(/^\s*\d+\.\s/)) {
            return line.replace(/^(\s*)(\d+\.)(\s)/, `$1${colors.dim}$2${colors.reset}$3`);
        }
        // Inline code `code` - highlight
        if (line.includes('`')) {
            return line.replace(/`([^`]+)`/g, `${colors.gold}$1${colors.reset}`);
        }
        // Bold **text** - make bold
        if (line.includes('**')) {
            return line.replace(/\*\*([^*]+)\*\*/g, `${colors.bold}$1${colors.reset}`);
        }
        return line;
    })
        .join('\n');
}
// Format AI response for blessed TUI (uses blessed tags)
function responseTui(text) {
    return text
        .split('\n')
        .map(line => {
        // Code blocks
        if (line.startsWith('```')) {
            return `{gray-fg}${line}{/gray-fg}`;
        }
        // Headers - bold with 256 color orange
        if (line.match(/^#{1,3}\s/)) {
            return `{bold}{#ff8700-fg}${line.replace(/^#+\s*/, '')}{/#ff8700-fg}{/bold}`;
        }
        // Bullet points
        if (line.match(/^\s*[-*]\s/)) {
            return line.replace(/^(\s*)([-*])(\s)/, `$1{#ff8700-fg}${exports.icons.bullet}{/#ff8700-fg}$3`);
        }
        // Numbered lists
        if (line.match(/^\s*\d+\.\s/)) {
            return line.replace(/^(\s*)(\d+\.)(\s)/, `$1{gray-fg}$2{/gray-fg}$3`);
        }
        // Inline code - gold/yellow
        if (line.includes('`')) {
            return line.replace(/`([^`]+)`/g, `{#ffaf00-fg}$1{/#ffaf00-fg}`);
        }
        // Bold
        if (line.includes('**')) {
            return line.replace(/\*\*([^*]+)\*\*/g, `{bold}$1{/bold}`);
        }
        return line;
    })
        .join('\n');
}
// ============================================================================
// TASK ANALYSIS OUTPUT
// ============================================================================
function taskType(type, complexity) {
    const colors = c();
    const typeColors = {
        chat: colors.gray,
        question: colors.blue,
        simple_edit: colors.green,
        complex_task: colors.magenta,
        debug: colors.red,
        refactor: colors.yellow,
    };
    const color = typeColors[type] || colors.white;
    return `${color}${type}${colors.reset} ${colors.dim}(${complexity})${colors.reset}`;
}
// ============================================================================
// BOX DRAWING
// ============================================================================
function box(lines, title) {
    const colors = c();
    const output = [];
    if (title) {
        output.push(`${colors.dim}─${colors.reset} ${colors.bold}${title}${colors.reset} ${colors.dim}─${colors.reset}`);
    }
    for (const line of lines) {
        output.push(`  ${line}`);
    }
    return output.join('\n');
}
// ============================================================================
// EXPORTS FOR TUI (blessed-compatible tags) - Cosmic Orange
// ============================================================================
exports.tuiTags = {
    success: (msg) => `{green-fg}${exports.icons.success}{/green-fg} ${msg}`,
    error: (msg) => `{red-fg}${exports.icons.error}{/red-fg} ${msg}`,
    warning: (msg) => `{#ffaf00-fg}${exports.icons.warning}{/#ffaf00-fg} ${msg}`,
    info: (msg) => `{#ff8700-fg}${exports.icons.info}{/#ff8700-fg} ${msg}`,
    dim: (msg) => `{gray-fg}${msg}{/gray-fg}`,
    thinking: (msg = 'Thinking') => `{#ff8700-fg}${exports.icons.thinking}{/#ff8700-fg} {gray-fg}${msg}...{/gray-fg}`,
    reading: (path) => `{gray-fg}${exports.icons.reading}{/gray-fg} {gray-fg}Reading{/gray-fg} ${path}`,
    writing: (path) => `{green-fg}${exports.icons.writing}{/green-fg} {gray-fg}Creating{/gray-fg} ${path}`,
    modifying: (path) => `{#ffaf00-fg}${exports.icons.modifying}{/#ffaf00-fg} {gray-fg}Modifying{/gray-fg} ${path}`,
    deleting: (path) => `{red-fg}${exports.icons.deleting}{/red-fg} {gray-fg}Deleting{/gray-fg} ${path}`,
    hint: (msg) => `{gray-fg}${exports.icons.arrow} ${msg}{/gray-fg}`,
    planStep: (i, desc, done = false) => {
        const icon = done ? `{green-fg}${exports.icons.success}{/green-fg}` : `{gray-fg}${exports.icons.pending}{/gray-fg}`;
        return `  ${icon} {gray-fg}${i}.{/gray-fg} ${desc}`;
    },
    fileOp: (op, path) => {
        const ops = {
            create: { icon: exports.icons.writing, color: 'green' },
            modify: { icon: exports.icons.modifying, color: '#ffaf00' },
            delete: { icon: exports.icons.deleting, color: 'red' },
        };
        const o = ops[op];
        return `{${o.color}-fg}${o.icon}{/${o.color}-fg} ${path}`;
    },
    fileResult: (path, ok) => {
        const icon = ok ? `{green-fg}${exports.icons.success}{/green-fg}` : `{red-fg}${exports.icons.error}{/red-fg}`;
        return `  ${icon} ${path}`;
    },
    summary: (stats) => {
        const parts = [];
        if (stats.created)
            parts.push(`{green-fg}+${stats.created}{/green-fg}`);
        if (stats.modified)
            parts.push(`{#ffaf00-fg}~${stats.modified}{/#ffaf00-fg}`);
        if (stats.deleted)
            parts.push(`{red-fg}-${stats.deleted}{/red-fg}`);
        return parts.join(' ');
    },
};
//# sourceMappingURL=theme.js.map