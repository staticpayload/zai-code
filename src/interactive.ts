...
const screen = {
  ...
  moveUp: (n: number) => process.stdout.write(`${CSI}${n}A`),
  moveDown: (n: number) => process.stdout.write(`${CSI}${n}B`),
...
// Render prompt
function renderPrompt(input: string): void {
...
function renderPalette(filter: string, selectedIndex: number): number {
  const { rows, cols } = getTermSize();
...
// Clear palette
function clearPalette(count: number): void {
...
// Add output line
function addOutput(line: string): void {
